...
- **FR-005**: 系统 MUST 支持定义“测试用例库”，每条用例至少包含：用例 ID、关联 UI 场景 ID、用例类别（正例/反例）、用例类型/标签、自然语言测试指令（prompt）、以及 Ground Truth（是否存在缺陷、缺陷细节、严重程度等）。
...
- **FR-009**: 系统 MUST 将 Agent 的输出规范化为统一结构，至少包含：是否存在缺陷、缺陷列表（每项含缺陷类型/描述/严重程度/证据或定位信息）、以及置信度（若 Agent 提供）。
...
- **Ground Truth**: 标准答案（是否有缺陷、缺陷项集合、严重程度/分级、可选证据/说明）。
